...
import { ScenarioCards } from '../ticker/ScenarioCards'; // Safe to reuse
import { PublicRatingsTable } from './PublicRatingsTable';

import type { TickerData, NewsItem, AnalystRating } from '../../types/ticker';

// Define localized types to avoid deep imports if desired, or reuse shared types for data shape
// We rely on the shape passed from PublicResearchPage
interface PublicOverviewProps {
    risk_analysis: TickerData['risk_analysis'];
    market_data: Omit<TickerData['market_data'], 'history'> & {
        price: number;
...
export function PublicTickerOverview({ risk_analysis, market_data, ratings, profile, news }: PublicOverviewProps) {
    ...
    return (
        <div className="flex flex-col gap-6">
            {/* Breaking News Alert - STATIC LINK ONLY (or no link) */}
            {news && (news.score || 0) >= 5 && isFresh && (
                <section className="relative">
                    {/* No internal navigation wrapper */}
                    <div className="relative rounded-xl border p-0.5 overflow-hidden">
                        <PublicNewsCardContent news={news} timeAgo={timeAgo} />
                    </div>
                </section>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <Card className="border-green-500/20 bg-green-500/5">
                    ...
                    <CardContent className="px-3 pb-3 pt-0">
                        <ul className="space-y-1.5">
                            {risk_analysis?.catalysts?.map((c, i: number) => (
                                <li key={i} className="text-sm text-foreground/80 flex items-start gap-2 leading-snug">
...
function PublicNewsCardContent({ news, timeAgo }: { news: NewsItem, timeAgo: string }) {
