...
export function GlobalSearch({ className = '', autoFocus = false, onSelect }: GlobalSearchProps) {
    ...
    // Reset external search flag on query change
    useEffect(() => {
        ...
        <div className={cn("relative w-full", className)} ref={wrapperRef}>
            <div className="relative">
                ...
                <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-2 pointer-events-none">
                    {/* Only show spinner here if NOT in dropdown mode, or maybe always? 
                        Detailed loader in dropdown is better for external search context.
                    */}
                    {!isOpen && isLoading && (
            ...
            {isOpen && query.trim().length > 0 && (
                <div className="absolute top-full mt-2 w-full bg-popover !bg-opacity-100 border border-border rounded-xl shadow-2xl z-50 overflow-hidden animate-in slide-in-from-top-2 !opacity-100">
                    ...
                    {/* Loading State */}
                    {isLoading ? (
                        <div className="px-4 py-8 flex items-center justify-center text-muted-foreground gap-3">
                            <Loader2 className="animate-spin w-5 h-5 text-primary" />
                            <span className="text-sm">Searching global markets...</span>
                        </div>
                    ) : results.length > 0 ? (
                        /* Results List */
                                            ...
                                            <div className="flex items-center gap-2 shrink-0 h-6">
                                                {ticker.is_locally_tracked && ticker.sparkline && ticker.sparkline.length > 0 ? (
                                                    ...
                                                    <button
                                                        ...
                                                        className="text-[10px] bg-blue-600 text-white border border-transparent hover:bg-blue-700 dark:bg-blue-500/10 dark:text-blue-400 dark:border-blue-500/20 dark:hover:bg-blue-500 dark:hover:text-white font-semibold px-2.5 py-0.5 rounded-full flex items-center gap-1.5 transition-all disabled:opacity-50 shadow-sm"
                                                    >
                        ...
                        /* Empty State: Search External OR No Results */
                        <div className="py-1">
                            {hasSearchedExternal ? (
                                <div className="px-4 py-8 text-center">
                                    <Search className="w-8 h-8 text-muted-foreground/30 mx-auto mb-2" />
                                    <p className="text-sm font-medium text-foreground">No matches found</p>
                                    <p className="text-xs text-muted-foreground">We couldn't find "{query}" in global markets.</p>
                                </div>
                            ) : (
                                <div
