...
export function PortfolioAiAnalyzer({ open, onOpenChange }: PortfolioAiAnalyzerProps) {
  ...
  const startAnalysis = async () => {
    ...
    try {
      ...
      const message = error.response?.data?.message || 'Failed to generate AI analysis';
      toast.error(message);
      setStep('survey');
    }
  };
        ...
        <div className="flex-1 overflow-y-auto py-6">
          ...
          {step === 'result' && (
             <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 prose prose-invert prose-sm max-w-none px-2">
                <ReactMarkdown
                  components={{
                    h1: ({node, ...props}) => <h1 className="text-primary font-bold text-lg mb-2" {...props} />,
                    h2: ({node, ...props}) => <h2 className="text-foreground font-bold text-base mt-4 mb-2 flex items-center gap-2" {...props} />,
