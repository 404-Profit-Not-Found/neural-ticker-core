...
describe('StockTwitsService', () => {
  ...
  describe('analyzeComments', () => {
    ...
    it('should analyze posts and save analysis + events', async () => {
      ...
      // 2. Mock Ticker
      mockTickersService.findOneBySymbol.mockResolvedValue({ id: 'ticker-123', symbol: 'AAPL' });
      ...
      const mockLlmResponse = {
        answerMarkdown: JSON.stringify({
          ...
          highlights: { topics: ['Earnings'], bullish_points: [], bearish_points: [] },
          extracted_events: [
            { title: 'Earnings Call', date: '2025-01-01', type: 'earnings', confidence: 0.95 }
          ]
        }),
        models: ['gemini-pro'],
        tokensIn: 1000,
      ...
      jest.spyOn(llmService, 'generateResearch').mockResolvedValue(mockLlmResponse as any);

      // 4. Run
      const result = await service.analyzeComments('AAPL');

      // 5. Verify Analysis Saved
      expect(analysisRepo.create).toHaveBeenCalledWith(expect.objectContaining({
        symbol: 'AAPL',
        sentiment_score: 0.9,
        tokens_used: 1200,
