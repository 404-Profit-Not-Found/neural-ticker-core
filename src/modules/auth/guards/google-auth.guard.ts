...
export class GoogleAuthGuard extends AuthGuard('google') {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
      ...
      console.log('[GoogleAuthGuard] Using options:', JSON.stringify(options, null, 2));

      // Manually authenticate using passport to ensure options (like state) are respected
      return new Promise<boolean>((resolve, reject) => {
          const authenticateFn = passport.authenticate('google', options, (err, user, info) => {
              if (err) {
              ...
              if (!user) {
                  ...
                  if (request.query.code) {
                      // Phase 2: Callback
                      console.error('[GoogleAuthGuard] No user found in callback phase');
                  ...
                  console.log('[GoogleAuthGuard] No user (likely redirecting or failed)');
