...
import { ApiTags, ApiOperation, ApiProperty, ApiResponse, ApiParam } from '@nestjs/swagger';
import { ResearchService } from './research.service';
import { QualityTier } from '../llm/llm.types';
import { IsArray, IsString, IsEnum, IsOptional, IsNumber } from 'class-validator';

class AskResearchDto {
  @ApiProperty({ example: ['AAPL'], description: 'List of tickers to research' })
  @IsArray()
  @IsString({ each: true })
  tickers: string[];

  @ApiProperty({ example: 'Is this a good long-term hold?', description: 'Research question' })
  @IsString()
  question: string;
...
export class ResearchController {
  ...
  @ApiOperation({ summary: 'Ask a research question to LLMs', description: 'Submits a research question about specific tickers to the configured LLM provider. Returns a generic ID to poll for results or the immediate result if synchronous.' })
  @ApiResponse({ 
      status: 201, 
  ...
  @ApiOperation({ summary: 'Retrieve a research note by ID', description: 'Fetches a previously asked research note.' })
