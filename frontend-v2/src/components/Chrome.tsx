...
import { StatusLED } from './pixel.tsx';

interface BootLine {
  text: string;
  color: 'info' | 'dim' | 'ok' | 'warn';
  delay?: number;
}
      ...
      <div className="boot-content">
        ...
        {shown < lines.length && (
          <div className="boot-line">
            <span className="pxl-cursor"></span>
          </div>
        )}
      </div>
    </div>
...
export function StatusBar({ route }: { route: string }) {
  ...
  return (
    <div className="status-bar">
      ...
      <span className="sep" />
      <span>
        <span className="faint">CTX</span> {route.toUpperCase()}
      </span>
      <span style={{ flex: 1 }} />
      <span>
        <span className="faint">FPS</span> {fps}
      </span>
      <span className="sep" />
      <span>
        <span className="faint">MEM</span> {memory}MB
      </span>
      <span className="sep" />
      <span>
        <span className="faint">UTC</span> {time}
      </span>
      <span className="sep" />
