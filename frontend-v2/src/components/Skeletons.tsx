...
// Converted from skeletons.jsx — no window globals.

import { useState, useEffect, type CSSProperties, type ReactNode } from 'react';
import { PixelIcon, PixelPanel } from './pixel.tsx';
...
export function Skel({
  ...
  const widthStyle = typeof w === 'number' ? `${w}px` : (w as string) || '100%';
  return (
    <span
      className="pxl-skel"
      style={{
  ...
  return (
    <div className="col gap-2" style={{ width: '100%' }}>
      {Array.from({ length: lines }).map((_, i) => (
        <Skel key={i} w={i === lines - 1 ? lastWidth : '100%'} h={10} />
      ))}
    </div>
  );
}

export function SkelDither({
  w = '100%',
  h = 60,
}: {
  w?: number | string;
...
export function Spinner({ label }: { label?: ReactNode }) {
  return (
    <span className="row gap-2" style={{ alignItems: 'center' }}>
      ...
      {label && (
        <span
          className="font-display t-xs amber"
...
export function OpportunityCardSkel() {
  return (
    <div className="pxl pxl-raised" style={{ padding: 12 }}>
...
export function WatchlistRowSkel() {
  return (
    <div
      className="row"
      style={{
...
export function TableRowSkel({ cols = 10 }: { cols?: number }) {
  return (
    <tr>
...
export function TypedLine({
  ...
  useEffect(() => {
    ...
    const id = setTimeout(
      () => setShown((s) => Math.min(s + charsPerTick, text.length)),
      tickMs,
    );
    return () => clearTimeout(id);
  }, [started, shown, text.length, charsPerTick, tickMs]);
  ...
  return (
    <div style={{ color: done ? 'var(--green)' : 'var(--ink)' }}>
      {text.slice(0, shown)}
      {!done && (
        <span className="pxl-cursor" style={{ marginLeft: 0 }}>
          &nbsp;
        </span>
      )}
    </div>
...
export function DonutSkel({ size = 140 }: { size?: number }) {
  return (
...
export function PanelSkel({
  ...
  title?: ReactNode;
  rows?: number;
  kind?: 'row' | 'card' | 'chart' | 'loading';
}) {
  return (
    <PixelPanel
      title={title || <Skel w={120} h={10} />}
      accent="cyan"
    ...
    >
      ...
      {kind === 'row' &&
        Array.from({ length: rows }).map((_, i) => <NewsRowSkel key={i} />)}
      {kind === 'chart' && (
...
export function LoadingOverlay({ label = 'SYNCING' }: { label?: string }) {
  return (
    <div
      style={{
        position: 'absolute',
    ...
    >
      <div
        ...
        style={{ padding: 20, minWidth: 240, textAlign: 'center' }}
      >
        <Spinner label={label} />
        <div className="mt-3">
          <SkelBar segments={10} />
...
export function EmptyState({
  icon = 'search',
  title = 'NO RESULTS',
  subtitle = 'Try a different filter or query.',
}: {
  icon?: string;
  title?: string;
  subtitle?: string;
