...
type ToastTone = 'amber' | 'green';
type ToastKind = 'alert' | 'research';
...
// Permissive shapes — backend may evolve; we only read a few fields here.
interface AlertRow {
  id: string | number;
  symbol: string;
  alert_type?: string;
...
export interface ToastBusProps {
  signedIn: boolean;
  onNav: NavFn;
}

export function ToastBus({ signedIn, onNav }: ToastBusProps) {
  const [toasts, setToasts] = useState<Toast[]>([]);
  const lastSeenAlertsRef = useRef<number | null>(null);
  const lastSeenResearchRef = useRef<number | null>(null);

  const [alerts] = useApi<AlertRow[] | null>(
    () => (signedIn ? (API.alerts() as Promise<AlertRow[]>) : Promise.resolve([])),
    ['toast', signedIn],
    { poll: 60_000, staleTime: 30_000 },
  );

  const [completed] = useApi<ResearchListResponse | null>(
    () =>
      signedIn
        ? (API.researchList({ status: 'completed', limit: 10 }) as Promise<ResearchListResponse>)
        : Promise.resolve(null),
    ['toast-research', signedIn],
    { poll: 120_000, staleTime: 60_000 },
  );

  // Initialise "last seen" from sessionStorage once per sign-in.
  useEffect(() => {
    if (!signedIn) return;
    try {
      lastSeenAlertsRef.current =
        Number(sessionStorage.getItem('v2_toast_seen_alerts')) || Date.now();
      lastSeenResearchRef.current =
        Number(sessionStorage.getItem('v2_toast_seen_research')) || Date.now();
    } catch {
  ...
  useEffect(() => {
    if (
      ...
      lastSeenAlertsRef.current == null
    )
      return;
    ...
    try {
      sessionStorage.setItem('v2_toast_seen_alerts', String(maxSeen));
    } catch {
      /* ignore */
    }
    ...
    for (const f of fresh) {
      setTimeout(
        () => setToasts((p) => p.filter((x) => x.id !== f.id)),
        6000,
      );
    }
  }, [alerts, signedIn]);
  ...
  useEffect(() => {
    if (
      ...
      lastSeenResearchRef.current == null
    )
      return;
    ...
    for (const r of completed.data) {
      ...
      fresh.push({
        ...
        sym: Array.isArray(r.tickers) && r.tickers[0] ? r.tickers[0] : null,
        researchId: r.id,
      });
      if (ts > maxSeen) maxSeen = ts;
    }
    ...
    try {
      sessionStorage.setItem('v2_toast_seen_research', String(maxSeen));
    } catch {
      /* ignore */
    }
    setToasts((prev) => [...prev, ...fresh].slice(-5));
    ...
    >
      {toasts.map((t) => (
        <div
          ...
          onClick={() => {
            if (t.sym) {
              // Construct a minimal Ticker stub for navigation; the detail
              // screen will hydrate from the backend on mount.
              const stub: Ticker = {
                sym: t.sym,
                name: t.sym,
                sector: '—',
        ...
        >
          ...
          <div className="font-mono t-sm" style={{ fontWeight: 700 }}>
            {t.title}
          </div>
