...
function FavoriteStar({ sym }) {
  const [filled, setFilled] = useStateA(null); // unknown until we toggle once
  ...
  const onClick = async (e) => {
    ...
    setBusy(true);
    const res = await window.API.favoritesToggle(sym).catch(() => null);
    setBusy(false);
...
function Analyzer({ onNav }) {
  ...
  const liveTickers = window.useTickersWithFallback({ limit: 100 });
  const m = Object.assign({}, window.MOCK, { tickers: liveTickers });
