...
function Workspace({ onNav }) {
  ...
  const tickers = window.useTickersWithFallback
    ? window.useTickersWithFallback({ limit: 50 })
    : window.MOCK.tickers;
...
function ChartPane({ t }) {
  ...
  const [live] = window.useTickerHistory
    ? window.useTickerHistory(t?._live ? t.sym : null, 60)
    : [null];
...
function NewsPane({ sym }) {
  const [live] = window.useTickerNews ? window.useTickerNews(sym) : [null];
