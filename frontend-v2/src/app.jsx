...
// ─────────────────────────────────────────────────────────────
function SyncingDot() {
  const fetching = window.useIsFetching ? window.useIsFetching() : 0;
...
function NotificationsBell({ signedIn, onNav }) {
  ...
  const [readKey, setReadKey] = useStateApp(() => {
    try { return Number(localStorage.getItem("v2_notif_read_until")) || 0; }
    catch (e) { return 0; }
  });

  const [alerts] = window.useApi
    ? window.useApi(() => (signedIn ? window.API.alerts() : Promise.resolve([])), [signedIn], { poll: 60000, staleTime: 30000 })
    : [null];
  const [completed] = window.useApi
    ? window.useApi(() => (signedIn ? window.API.researchList({ status: "completed", limit: 5 }) : Promise.resolve(null)), [signedIn], { staleTime: 60000 })
    : [null];
...
function Header({ route, onNav, query, setQuery }) {
  ...
  const [searchResults, setSearchResults] = useStateApp(null);
  const bp = useBreakpoint();

  // Real user from /auth/profile; null when unauth.
  const [profile, profileMeta] = window.useApi
    ? window.useApi(() => window.API.profile(), [])
    : [null, {}];
  ...
  useEffectApp(() => {
    ...
    const t = setTimeout(() => {
      if (window.API && signedIn) {
        window.API.tickers(query).then((rows) => {
          if (abort) return;
          ...
          <div style={{ padding: "0 0 10px", position: "relative" }}>
              ...
              onKeyDown={e => {
                if (e.key === "Enter" && query) {
                  const t = (searchResults && searchResults[0]) ||
                            window.MOCK.tickers.find(x => x.sym.toLowerCase() === query.toLowerCase());
            ...
            {query && (
              <div className="pxl pxl-raised" style={{ marginTop: 8 }}>
                {(searchResults || window.MOCK.tickers
...
function LoginModal({ onClose }) {
  const [devEmail, setDevEmail] = useStateApp("dev@test.com");
  ...
  const runDevLogin = async () => {
    ...
    try {
      const res = await window.API.devLogin(devEmail);
...
function App() {
  ...
  const [signInToast, setSignInToast] = useStateApp(false);
  ...
  const paletteTickers = window.useTickersWithFallback
    ? window.useTickersWithFallback({ limit: 80 })
    : window.MOCK.tickers;

  // Profile (shared via query cache with Header's hook)
  const [appProfile] = window.useApi
    ? window.useApi(() => window.API.profile(), [])
    : [null];
  ...
  const handleAction = React.useCallback((a) => {
    if (a === "replayBoot") {
  ...
  if (window.useGlobalShortcuts) {
    window.useGlobalShortcuts({
