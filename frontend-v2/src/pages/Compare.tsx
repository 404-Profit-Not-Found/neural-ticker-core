...
export function ComparePage({ onNav }: ComparePageProps) {
  const bp = useBreakpoint();
  const liveTickers = useTickersWithFallback({ limit: 100 });
  const tickers: Ticker[] =
    Array.isArray(liveTickers) && liveTickers.length > 0
      ? liveTickers
      : MOCK.tickers;
  ...
  useEffect(() => {
    ...
    if (stale) {
      ...
      const next = picked.map((s) =>
        known.has(s) ? s : fresh.shift() || tickers[0].sym,
      );
      setPicked(next);
    }
  ...
  useEffect(() => {
    try {
      localStorage.setItem('v2_compare_picks', JSON.stringify(picked));
    } catch {
      /* ignore */
    }
  }, [picked]);
  ...
  const byMap = useMemo(
    () => new Map<string, Ticker>((tickers || []).map((t) => [t.sym, t])),
    [tickers],
  );
  const selected: Ticker[] = picked
    .map((s) => byMap.get(s))
    .filter((t): t is Ticker => !!t);

  const symbolOptions = (tickers || []).map((t) => t.sym);

  const setSlot = (idx: number, sym: string): void => {
    setPicked((p) => {
      const next = [...p];
      if (sym === '') {
        next.splice(idx, 1);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {picked.length < 4 && (
          <button className="pxl-btn sm primary" onClick={addSlot}>
            + ADD
          </button>
        )}
      </div>
        ...
        <>
          ...
          <div className="pxl pxl-raised" style={{ padding: 14 }}>
            ...
            >
              ...
              <CompareRow
                label="PRICE"
                cells={selected.map((t) => `$${(t.price || 0).toFixed(2)}`)}
              />
              <CompareRow
                label="CHANGE %"
                cells={selected.map((t) => ({
              ...
              <CompareRow
                label="AI VERDICT"
                cells={selected.map((t) => ({ verdict: t.ai }))}
              />
              <CompareRow
                label="UPSIDE"
                cells={selected.map((t) => ({
              ...
              <CompareRow
                label="RISK SCORE"
                cells={selected.map((t) => `${t.risk}/10`)}
              />
              <CompareRow
                label="MKT CAP"
                cells={selected.map((t) => t.mc || '—')}
              />
              <CompareRow
                label="P/E"
                cells={selected.map((t) =>
                  t.pe != null ? Number(t.pe).toFixed(1) : '—',
                )}
              />
              <CompareRow
                label="SECTOR"
          ...
          {/* Sparklines panel */}
          <PixelPanel
            title="30-DAY PRICE TRAJECTORY (NORMALIZED)"
            accent="cyan"
          >
...
export function CompareRow({ label, cells }: CompareRowProps) {
  ...
  return (
    <>
      <div
        className="font-display t-xs faint"
        style={{ padding: '10px 0', borderTop: '1px dashed var(--line)' }}
      >
        {label}
      </div>
      {cells.map((c, i) => {
        if (c && typeof c === 'object' && 'verdict' in c) {
          return (
        ...
        if (c && typeof c === 'object' && 'tone' in c) {
          return (
...
export interface CompareSparklinesProps {
  tickers: Ticker[];
}

export function CompareSparklines({ tickers }: CompareSparklinesProps) {
  const width = 720;
  const height = 180;
  const pad = 10;
  ...
  if (allVals.length === 0) {
    return (
      ...
      >
        No sparkline data for selection.
      </div>
    );
  }
    ...
    <div>
      ...
      >
        ...
        {series.map((s, idx) => {
          ...
          const points: Array<[number, number]> = s.map((v, i) => {
