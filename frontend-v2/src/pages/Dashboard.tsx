...
function MarketTicker() {
  ...
  const [live] = useApi(() => API.indices(), [], { poll: 30000 });
...
function PortfolioAIWidget() {
  const [data, meta] = useApi(() => API.portfolioAnalyses(), []);
...
function Dashboard({ onNav, loading = false }) {
  ...
  const liveTickers = useTickersWithFallback({ limit: 50 });
  // Live counts + portfolio + news + alerts (all gracefully null on failure).
  const [tickerCount] = useApi(() => API.tickerCount(), []);
  const [strongBuy]   = useApi(() => API.statsStrongBuy(), []);
  const [sellStats]   = useApi(() => API.statsSell(), []);
  const [livePortfolio] = useApi(() => API.portfolio(), []);
  const [liveNews]      = useApi(() => API.newsGeneral(), []);
  const [liveDigest]    = useApi(() => API.newsDigest(), []);
  const [liveAlerts]    = useApi(() => API.alerts(), []);
  const [liveWatchlists] = useApi(() => API.watchlists(), []);
  ...
  const m = Object.assign({}, MOCK, { tickers: liveTickers, portfolio });
...
export { MarketTicker };
export { AIDigest };
export { AllocationPanel };
export { PortfolioAIWidget };
