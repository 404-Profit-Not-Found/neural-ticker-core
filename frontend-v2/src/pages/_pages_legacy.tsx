...
function PortfolioPage({ onNav }) {
  ...
  const [positions, posMeta] = useApi(
    () => API.portfolio(currency || undefined),
    [currency],
  );
...
function ResearchPage({ onNav }) {
  ...
  const [data, meta] = useApi(
    () => API.researchList({ status: filter, page, limit: 30 }),
    [filter, page],
  );
...
function AlertsPage({ onNav }) {
  const bp = useBreakpoint();
  const [data, meta] = useApi(() => API.alerts(), []);
...
function NewsPage() {
  const bp = useBreakpoint();
  const [data, meta] = useApi(() => API.newsGeneral(), [], { poll: 60000 });
  const items = Array.isArray(data) ? data : [];
...
function WatchlistPage({ onNav }) {
  const bp = useBreakpoint();
  const [data, meta] = useApi(() => API.watchlists(), []);
  ...
  const createList = async () => {
    ...
    if (!name) return;
    setBusy(true);
    const res = await API.watchlistCreate(name).catch(() => null);
    setBusy(false);
...
function PushControls() {
  ...
  const subscribe = async () => {
    ...
    try {
      ...
      // 2. Get VAPID key
      const v = await API.pushVapidKey();
...
function AdminPage() {
  const bp = useBreakpoint();
  const [users, meta] = useApi(() => API.adminUsers(), []);
  ...
  const grantCredits = async (u) => {
    ...
    setBusy(true);
    const res = await API.adminGrantCredits(u.id, amount, "admin_gift").catch(() => null);
    setBusy(false);
  ...
  const changeRole = async (u) => {
    ...
    setBusy(true);
    const res = await API.adminSetRole(u.id, role).catch(() => null);
    setBusy(false);
  ...
  const changeTier = async (u) => {
    ...
    setBusy(true);
    const res = await API.adminSetTier(u.id, tier).catch(() => null);
    setBusy(false);
...
function UploadResearchDialog({ onClose }) {
  ...
  const submit = async () => {
    ...
    setBusy(true);
    const res = await API.researchContribute(tickers, content)
      .catch((e) => ({ _err: e }));
    setBusy(false);
    if (!res || res._err) {
...
function PublicReportPage({ shareKey, onNav }) {
  ...
  const [data, meta] = useApi(
    () => (id && signature ? API.researchPublic(id, signature) : Promise.resolve(null)),
    [id, signature],
  );
...
export { TermsPage };
export { PrivacyPage };
export { EditPositionDialog };
export { UploadResearchDialog };
