...
function TickerDetail({ t, onBack }) {
  ...
  const [liveCandles, candlesMeta] = window.useTickerHistory
    ? window.useTickerHistory(t._live ? t.sym : null, rangePresetOpts(range))
    : [null, {}];
  const [liveNews] = window.useTickerNews
    ? window.useTickerNews(t._live ? t.sym : null)
    : [null];
  const [composite] = window.useTickerComposite
    ? window.useTickerComposite(t._live ? t.sym : null)
    : [null];
      ...
      <div className="row" style={{ alignItems: "center", gap: 8, padding: "12px 0", flexWrap: "wrap" }}>
        ...
        <button
          ...
          onClick={async () => {
            ...
            const res = await window.API.favoritesToggle(t.sym).catch(() => null);
            // Invalidate watchlist queries so sidebar + watchlist page refresh
            if (window.invalidateQueries) window.invalidateQueries("/watchlists");
            if (res?.added != null) {
...
function ResearchTab({ t, bp }) {
  ...
  const [liveData] = window.useApi
    ? window.useApi(
        () => (t._live && window.API
          ? window.API.researchList({ ticker: t.sym, limit: 12, status: "all" })
...
function SocialTab({ t, bp }) {
  ...
  const [liveComments, commentsMeta] = window.useApi
    ? window.useApi(
        () => (t._live && window.API ? window.API.socialComments(t.sym) : Promise.resolve(null)),
        [t.sym, t._live],
      )
    : [null, {}];
  const [watchersData] = window.useApi
    ? window.useApi(
        () => (t._live && window.API ? window.API.socialWatchers(t.sym) : Promise.resolve(null)),
        [t.sym, t._live],
      )
    : [null];

  // StockTwits public sentiment + watchers history (no auth required).
  const [stAnalysis] = window.useApi
    ? window.useApi(
        () => (t._live && window.API ? window.API.stocktwitsAnalysis(t.sym) : Promise.resolve(null)),
        [t.sym, t._live],
      )
    : [null];
  const [stWatchers] = window.useApi
    ? window.useApi(
        () => (t._live && window.API ? window.API.stocktwitsWatchers(t.sym) : Promise.resolve(null)),
        [t.sym, t._live],
      )
    : [null];
...
function AlertCreatePanel({ t, onClose, onDone }) {
  ...
  const submit = async () => {
    ...
    setBusy(true);
    const res = await window.API.alertCreate({
      symbol: t.sym,
      alert_type: type,
      target_value: num,
    }).catch((e) => ({ _err: e }));
...
function BuyPositionPanel({ t, onClose, onDone }) {
  ...
  const submit = async () => {
    ...
    setBusy(true);
    const res = await window.API.portfolioCreate({
      symbol: t.sym,
      shares: n,
      buy_price: p,
      buy_date: date,
...
function ReportHistoryRow({ r }) {
  ...
  const onShare = async (e) => {
    e.stopPropagation();
    if (!r.id || !window.API) return;
    setShareBusy(true);
    const res = await window.API.researchPublicLink(r.id).catch(() => null);
    setShareBusy(false);
...
function RunResearchPanel({ t, onClose, onDone }) {
  ...
  const submit = async () => {
    ...
    const res = await window.API.researchAsk({
      tickers: [t.sym],
      question,
      provider,
      quality,
  ...
  const pollUntilDone = async (id) => {
    ...
    const tick = async () => {
      attempt++;
      const r = await window.API.researchGet(id).catch(() => null);
